<?php
/**
 * GET /wp-api/case-study.php?slug=your-case-study-slug
 *
 * A single published case study ("portfolio-item" post type) by slug,
 * with full content. Same shape as post.php.
 */
...
try {
    $stmt = $pdo->prepare("
        ...
        FROM {$prefix}posts
        WHERE post_name = :slug AND post_type = 'portfolio-item' AND post_status = 'publish'
        LIMIT 1
    ");
    $stmt->execute(['slug' => $slug]);
    $row = $stmt->fetch();
} catch (Throwable $e) {
    error_log('WP API case study query failed: ' . $e->getMessage());
    json_error('Unable to fetch case study.', 500);
}

if (!$row) {
    json_error('Case study not found.', 404);
}
